...
> 2026-09-19 实现更新：目前展示明确标注边界的“本站信誉参考分”，不称为总纯净度或风险概率；缺失维度不补默认分，并单独展示来源覆盖与分歧。现行规则见 [ip-quality-scoring.md](./ip-quality-scoring.md)。本节保留原始指标不可直接等同的分析，后面的展示方案属于历史建议。
